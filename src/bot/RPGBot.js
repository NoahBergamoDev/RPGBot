...
const { prefix } = config; //loading config file with token and prefix
const fs = require("fs"); //this package is for reading files and getting their inputs

const setupBot = (client, controllers) => {
  ...
  client.categories = fs.readdirSync("./src/bot/commands/"); //categories
...
const messageHandler = (client, controllers) => {
  ...
  client.on("message", async (message) => {
    ...
    if (command) {
      ...
      return message.reply(
        `Sorry, I must have missed something. Try again or try: **\`${prefix}help\`** to see if you're not mistaken.`
      );
  });
};

module.exports = async (controllers) => {
  ...
  setupBot(client, controllers);
  messageHandler(client, controllers);
