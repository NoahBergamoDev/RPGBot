...
module.exports = (client) => {
  readdirSync("./src/bot/commands/").forEach((dir) => {
    ...
    const commands = readdirSync(`./src/bot/commands/${dir}/`).filter((file) =>
      file.endsWith(".js")
    ); //it will be only a command if it ends with .js
    for (let file of commands) {
      ...
      if (pull.name) {
        ...
        ); //if something wents wrong, do this
        continue; //and skip
      }
      if (pull.aliases && Array.isArray(pull.aliases))
        pull.aliases.forEach((alias) => client.aliases.set(alias, pull.name)); //if there are aliases, do it too
    }
  });
